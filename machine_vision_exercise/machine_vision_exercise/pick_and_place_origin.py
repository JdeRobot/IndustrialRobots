...
class Pick_Place:
    ...
    # pick up object
    def pickup(self, object_name, position, distance = 0.15):
        if not self.is_inside_workspace(position.x, position.y, position.z):
            rospy.loginfo('***** GOAL POSE IS OUT OF ROBOT WORKSPACE *****')
    ...
    def place(self, object_name, position, distance=0.1):
        # Step 1: Check if the position is inside workspace
        if not self.is_inside_workspace(position.x, position.y, position.z):
            rospy.loginfo('***** GOAL POSE IS OUT OF ROBOT WORKSPACE *****')
            rospy.loginfo('Stop placing')
            return False

        # Step 2: Raise the placement height slightly to avoid table collision
        position.z = 0.1
        ...
        pose.orientation = Quaternion(*q)
        pose.position.z += distance  # offset above target

        # Step 4: Define constraints
        constraint = Constraints()
        joints = self.get_joints_value()

        joint_names = ["wrist_2_joint", "wrist_3_joint", "wrist_1_joint", "elbow_joint"]
        joint_indices = [4, 5, 3, 2]
        limits = [0.8, 0.8, 1.0, 1.2]
        weights = [0.2, 0.2, 0.1, 0.1]

        for i in range(4):
            jc = JointConstraint()
            jc.joint_name = joint_names[i]
            jc.position = joints[joint_indices[i]]
            jc.tolerance_above = limits[i]
            jc.tolerance_below = limits[i]
            jc.weight = weights[i]
        ...
        # Step 5: Apply path constraints
        # self.arm.set_path_constraints(constraint)
        self.arm.set_max_velocity_scaling_factor(0.2)
        self.arm.set_workspace([-0.8, -0.8, -0.2, 0.8, 0.8, 0.3])

        # Step 6: Try to move above the target with retry mechanism
        ...
        for attempt in range(3):
            ...
            self.move_pose_arm(pose)
            rospy.sleep(0.2)
            if self.arm.get_current_pose().pose.position.z > 0.1:
                success = True
                break
            rospy.logwarn("Retrying move_pose_arm()... attempt {}".format(attempt + 1))

        if not success:
            rospy.logwarn("Failed to reach pre-place pose after retries")
            self.arm.clear_path_constraints()
            return False

        self.arm.clear_path_constraints()

        # Step 7: Move down to placement
        waypoints = []
        wpose = self.arm.get_current_pose().pose
        wpose.position.z -= distance
        waypoints.append(copy.deepcopy(wpose))
        ...
        if not result:
            rospy.logwarn("Placing motion (lowering) failed")
            return False

        # Step 8: Release the object
        self.gripper_release()
        ...
        result = self.arm.execute(plan, wait=True)
        if not result:
            rospy.logwarn("Placing motion (lifting) failed")
            return False

        # Step 10: Reset workspace and finish

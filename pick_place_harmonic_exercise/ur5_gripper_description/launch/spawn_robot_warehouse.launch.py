...
def generate_launch_description():
    ...
    resource_path = ur5_share_parent + ":" + robotiq_share_parent + ":" + warehouse_models_path
    ...
    robot_description_content = xacro.process_file(
        xacro_file,
        mappings={
            "ur_type": "ur5", "name": "ur", "prefix": "",
            "use_fake_hardware": "false", "sim_gazebo": "false", "sim_gz": "true",
    ...
    gazebo = ExecuteProcess(
        ...
        output="screen", additional_env=gz_env, shell=False
    )

    spawn_entity = Node(
        package="ros_gz_sim", executable="create",
        arguments=["-topic", "robot_description", "-name", "ur5_robotiq",
                   "-allow_renaming", "true", "-x", "0.0", "-y", "0.0", "-z", "0.9",
                   "-R", "0.0", "-P", "0.0", "-Y", "0.0"],
        output="screen",
    )
    ...
    move_group_node = Node(
        ...
        parameters=[robot_description, robot_description_semantic, kinematics_yaml,
                    ompl_planning_yaml, planning_pipelines_config, trajectory_execution,
                    moveit_controllers, planning_scene_monitor_parameters, {"use_sim_time": True}],
    ...
    rviz_node = Node(
        ...
        parameters=[robot_description, robot_description_semantic, ompl_planning_yaml,
                    kinematics_yaml, {"use_sim_time": True}],
        condition=IfCondition(LaunchConfiguration("launch_rviz")),
    )

    delay_spawn = TimerAction(period=5.0, actions=[spawn_entity])
    
    delay_jsb = RegisterEventHandler(OnProcessExit(target_action=spawn_entity, on_exit=[load_joint_state_broadcaster]))
    delay_jtc = RegisterEventHandler(OnProcessExit(target_action=load_joint_state_broadcaster, on_exit=[load_joint_trajectory_controller]))
    delay_gc = RegisterEventHandler(OnProcessExit(target_action=load_joint_trajectory_controller, on_exit=[load_gripper_controller]))
    delay_mg = TimerAction(period=10.0, actions=[move_group_node])
    delay_rviz = TimerAction(period=12.0, actions=[rviz_node])

    return LaunchDescription(declared_arguments + [
        gazebo, robot_state_publisher, static_tf, gz_ros2_bridge_clock,
        delay_spawn, delay_jsb, delay_jtc, delay_gc, delay_mg, delay_rviz,
    ])

...
#include <pcl_filter_msgs/ColorFilter.h>
#include <pcl_filter_msgs/ShapeFilter.h>
...
public:
  ...
  ObjectDetection()
  {
    ...
    sub_color_filter = nh.subscribe<pcl_filter_msgs::ColorFilter> ("/start_color_filter", 1, &ObjectDetection::call_color_filter, this);
    sub_shape_filter = nh.subscribe<pcl_filter_msgs::ShapeFilter> ("/start_shape_filter", 1, &ObjectDetection::call_shape_filter, this);
  }

  void call_color_filter(const pcl_filter_msgs::ColorFilter::ConstPtr& msg)
  {
    if(msg->status == true){
      // start color filter
      switch(msg->color){
  ...
  void call_shape_filter(const pcl_filter_msgs::ShapeFilter::ConstPtr& msg)
  {
    if(msg->status == true){
      ...
      else if(msg->color == YELLOW && msg->shape == SPHERE)
      {
        yellow_sphere_radius = msg->radius;
        sub_yellow_sphere = nh.subscribe<PointCloud>("/yellow_filter", 1, &ObjectDetection::yellow_sphere_callback, this);
      }
      else if(msg->color == YELLOW && msg->shape == CYLINDER)
      {
        yellow_cylinder_radius = msg->radius;
  ...
  void detect_cylinder(const PointCloud::ConstPtr& cloud, ros::Publisher pub, ros::Publisher image_pub, std::string frame_id, float radius)
  {
    ...
    if(cloud_filtered->points.size ()<10)
      {ROS_INFO("Cannot filter out desired shape");
      return;}
...
private:
  ...
  ros::Subscriber sub_color_filter;
  ros::Subscriber sub_shape_filter;
...
int main(int argc, char** argv)
{
  ...
  ObjectDetection od;
  // ros::ServiceServer service_color_filter = od.nh.advertiseService<ObjectDetection> ("color_filter_server", &ObjectDetection::call_color_filter);
  // ros::ServiceServer service_shape_filter = od.nh.advertiseService<ObjectDetection> ("shape_filter_server", &ObjectDetection::call_shape_filter);
  ros::spin();
}

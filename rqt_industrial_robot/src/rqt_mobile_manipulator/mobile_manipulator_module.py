...
class MobileManipulator(Plugin):
    def __init__(self, context):
        super(MobileManipulator, self).__init__(context)
        # Give QObjects reasonable names
        self.setObjectName('MobileManipulator')
        ...
        self._widget.gripperBrowser.append(str(home_value["gripper"]))
    ...
    def planexe(self):
        ...
        # print("start moving")
        # update after robot stops moving
        while abs(sum(last_joints)-sum(self.robot.get_joints_value()) > 0.1):
            last_joints = self.robot.get_joints_value()
            #print(last_joints)
            time.sleep(0.5)
